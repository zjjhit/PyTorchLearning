...
class BertForSequenceClassification(nn.Module):
    ...
    ```
    """
    def __init__(self, config, num_labels, layers=None, pooling=None):
        ...
        self.classifier = nn.Linear(
            config.hidden_size * max(1, len(self.layers) if self.pooling is None else 1), num_labels)
        # self.classifier = MLP(
        #     [config.hidden_size * max(1, len(self.layers) if self.pooling is None else 1), config.hidden_size, num_labels])
