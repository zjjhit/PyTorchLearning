...
def make_docs(wrong, correct):
    '''
    将 两个字符串 按照标点符号进行切分，并且严格要求 对应部分的长度 一致。 （仅针对等长字符的情况进行纠错）
...
def main(fname, output_dir):
    confusions = {}  ### 存储对应 错字+正确字 的统计信息

    for line in open(fname, 'r', encoding='utf-8'):
        tmp = line.strip().split('\t')
        if len(tmp) != 3 or len(tmp[1]) != len(tmp[2]):  ###无法预测下一句
            continue
        num, wrong, correct = tmp
        ...
        #     print(correct)
        #     exit()
        # assert len(wrong) == len(correct)  ### 仅能对等长的文本进行处理
        # num = int(num)

        make_docs(wrong, correct)  ### TODO :
        ...
        if num >= 2:  ### 当有2个以上不同的时候，随机生成fake wrong txt
            # if len(poses) != num:
            #     print(wrong)
            #     print(correct)
            #     exit()
    ...
    correct_count = {}
    for line_c, line_w in zip(open(os.path.join(args.output, 'correct.txt'), 'r', encoding='utf-8'),
                              open(os.path.join(args.output, 'wrong.txt'), 'r', encoding='utf-8')):
        if line_c.strip():
...
def parse_args():
    ...
    parser.add_argument('--file', '-f', type=str, default='../data/tmp.txt',
