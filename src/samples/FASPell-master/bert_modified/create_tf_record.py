...
flags.DEFINE_string("input_file", '../data/correct.txt',
                    "Input raw text file (or comma-separated list of files).")

flags.DEFINE_string("wrong_input_file", '../data/wrong.txt',
                    "same as input_file except containing wrong characters.")

flags.DEFINE_string(
    "output_file", '../data/tran.tf.recorde',
    "Output TF example file (or comma-separated list of files).")

flags.DEFINE_string("vocab_file", '../model/pre-trained/vocab.txt',
...
def create_training_instances(input_files, tokenizer, max_seq_length,
    ...
    for input_file in input_files:
        with tf.gfile.GFile(input_file, "r") as reader:
            while True:
                ...
                # Empty lines are used as document delimiters  ##  [ seg_list,...]  两层嵌套的意义？
                if not line:
    ...
    print('Len all_documents is %d  %s' % (len(all_documents), all_documents[0]))
    ...
    # Remove empty documents
    all_wrong_documents = [x for x in all_wrong_documents if x]  ## [[line_seg],...]

    assert len(all_documents) == len(all_wrong_documents)

    print('Len all_wrong_documents is %d  %s' % (len(all_wrong_documents), all_wrong_documents[0]))

    # rng.shuffle(all_documents)

    vocab_words = list(tokenizer.vocab.keys())
    instances = []
    for _ in range(dupe_factor):  ### copy times
...
def create_instances_from_document(
    ...
    document = all_documents[document_index]  ## list[word]
    ...
    current_length = 0
    i = 0
    while i < len(document):  ### list[word]
        segment = document[i]
        ...
        try:
            assert len(segment) == len(wrong_segment)  ###约束过强
        except:
            print('some thing wrong\t' + ' '.join(segment))
            print('some thing wrong\t' + " ".join(wrong_segment))
            i += 1
            continue
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                ### 在此之前的句子，均是随机打断， 拆成 tokens_a,tokens_b 两部分
...
def create_masked_lm_predictions_for_wrong_sentences(tokens, masked_lm_prob,
    ...
    num_to_predict = min(max_predictions_per_seq,
                         max(1, int(round(
                             len(tokens)))))  # we set 100% masking rate to allow all errors and corresponding non-errors to be masked
