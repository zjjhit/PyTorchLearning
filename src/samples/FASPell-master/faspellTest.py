    ...
    def make_corrections(self,
        ...
        for i, cand in progess_bar:
            ...
            if re.findall(r'[a-zA-ZＡ-Ｚａ-ｚ]+', sentences[i]):
                ...
                extended_cand = extension(cand)  # dim : real_len , 5 ,2
                    ...
                    if 0 < j < len(extended_cand) - 1:  # skip the head and the end placeholders -- `。`
                        ...
                        for rank in range(rank_in_question + 1):
                            ...
                            error = self.get_error(sentences[i],
                                                   ...
                                                   filter_is_on=filter_is_on, sim_type=sim_type)  ## rank yu  difference  决定了curxe
                            # 的过滤策略 ,如果过略 就返回 None
                            if not max_error:
                                max_error = error
                            else:
                ...
                # pick only the one with the greatest confidence.  XXX: res仅记录错误的字信息
                ...
                if tackle_n_gram_bias:
                    ...
                    res = [e for e in res if e["error_position"] not in error_delete_positions]  ###res中不在 error_delete中保留
                    ...
                    sentence = ''.join([process(pos, c) for pos, c in enumerate(sentence)])  ### 在error_delete 中的替换回去
