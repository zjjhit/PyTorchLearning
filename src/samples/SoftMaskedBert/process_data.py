...
from SoftMaskedBert.data_loader import load_dataset
...
if __name__ == '__main__':
    ...
    # save_data(process_dataset, 'data/process_data_765376.txt')
    ...
    # df = pd.read_csv('data/processed_data/processed_765376.csv')

    dataset = df.values
    train, test = train_test_split(dataset, test_size=0.1)
    df = pd.DataFrame(columns=['origin_text', 'random_text', 'label'], data=train)
    df.to_csv('data/processed_data/train.csv', index=False)
    df = pd.DataFrame(columns=['origin_text', 'random_text', 'label'], data=test)
    df.to_csv('data/processed_data/test.csv', index=False)
