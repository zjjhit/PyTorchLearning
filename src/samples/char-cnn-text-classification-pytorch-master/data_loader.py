...
class AGNEWs(Dataset):
    ...
    def oneHotEncode(self, idx):
        # X = (batch, 70, sequence_length)  X = (70 ,sequence_length)
        X = torch.zeros(len(self.alphabet), self.l0)
        sequence = self.data[idx][:self.l0]
        for index_char, char in enumerate(sequence[::-1]):  ### 字符逆序处理，index 从0 开始
...
if __name__ == '__main__':
    ...
    train_dataset = AGNEWs(label_data_path, alphabet_path)  ### item  是一个 list[ X , Y ]
    train_loader = DataLoader(train_dataset, batch_size=64, drop_last=False)

    print(train_dataset.getClassWeight())
    print(len(train_dataset.alphabet))

    # print(len(train_dataset))
    # # size = 0
    # for i_batch, sample_batched in enumerate(train_loader):
    #     if i_batch == 0:
    #         print(sample_batched.shape, len(sample_batched))
