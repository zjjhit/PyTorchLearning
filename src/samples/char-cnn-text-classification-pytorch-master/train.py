...
from Mmodel import CharCNN

# import .model.CharCNN
...
experiment.add_argument('--log_interval', type=int, default=500,
                        help='how many steps to wait before logging training status [default: 1]')
experiment.add_argument('--val_interval', type=int, default=500, help='how many steps to wait before vaidation [default: 200]')
experiment.add_argument('--save_interval', type=int, default=50, help='how many epochs to wait before saving [default:1]')
...
def train(train_loader, dev_loader, model, args):
    ...
    for epoch in range(start_epoch, args.epochs + 1):  # 200
        if args.dynamic_lr and args.optimizer != 'Adam':
            scheduler.step()
        for i_batch, data in enumerate(train_loader, start=start_iter):
            ...
            if i_batch % args.val_interval == 0:
                ...
                print('EVA - Epoch[{}] Batch[{}] - acc:{:.3f}'.format(epoch, i_batch, val_acc))
...
def test():
    ...
    data_dataset, data_loader = make_data_loader(args.test_path,
                                                 args.alphabet_path, args.l0, args.batch_size, args.num_workers)

    args.num_features = 70  # len(train_dataset.alphabet)

    _, num_class_dev = data_dataset.getClassWeight()

    file_path = '%s/CharCNN_epoch_200.pth.tar' % (args.save_folder)
    model = CharCNN(args)
