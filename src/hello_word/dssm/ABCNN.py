...
class Abcnn2(nn.Module):
    ...
    def __init__(self, emb_dim, sentence_length, filter_width, filter_channel=50, layer_size=2, match='cosine',
                 inception=True):
...
class InceptionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        out_2 = self.conv_2(x)
        out_3 = self.conv_3(x)
        print('InceptionModule', out_1.shape, out_2.shape, out_3.shape)
...
def convolution(in_channel, filter_width, filter_height, filter_channel, padding):
    '''convolution layer
    '''
    # print('convolution', in_channel, filter_channel)
    model = nn.Sequential(
